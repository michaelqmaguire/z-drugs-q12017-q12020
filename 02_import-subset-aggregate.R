...
## -------------------------------- ##
## ISSUE: GA MISSING DATA FOR 2018. ##
## JHC SAID TO REMOVE FOR NOW.      ##
## -------------------------------- ##
...
## -------------------------------------------------------------- ##
## Step 7: Aggregate by state, year, quarter and sum, without GA. ##
## -------------------------------------------------------------- ##
...
## ----------------------------------------------------------- ##
## Step 8: Plot number of prescriptions over time, without GA. ##
## ----------------------------------------------------------- ##

## Make plots for overall numbers.

rx_by_year_quarter_rm_ga <-
...
rx_by_year_quarter_state_rm_ga <-
  ggplot(data = sdud_2017_2020_zdrugs_rm_ga) +
  geom_col(aes(x = paste0(year, "-", quarter), y = total_prescriptions, fill = state), alpha = 0.95) +
  scale_fill_viridis_d() +
  scale_y_continuous(labels = scales::comma) + 
...
## ------------------------------------------ ##
## Step 9: Output data and plots, without GA  ##
## ------------------------------------------ ##

## Write out data to clean folder.

write_csv(x = ndc_gen_brand_names_rm_ga, file = "./data/clean/z-drugs-generic-brand-ndc-no-ga.csv")
write_csv(x = sdud_2017_2020_zdrugs_rm_ga, file = "./data/clean/z-drugs-q1-2017-q1-2020-year-quarter-state-no-ga.csv")
write_csv(x = sdud_2017_2020_zdrugs_rx_rm_ga, file = "./data/clean/z-drugs-q1-2017-q1-2020-year-quarter-state-drug-no-ga.csv")

## Save plots to the plots folder.

png(filename = "./plots/z-drugs-q1-2017-q1-2020-by-year-quarter-no-ga.png", width = 16, height = 9, units = "in", res = 1200)
rx_by_year_quarter_rm_ga
dev.off()

png(filename = "./plots/z-drugs-q1-2017-q1-2020-by-year-quarter-state-no-ga.png", width = 18, height = 12, units = "in", res = 1200)
rx_by_year_quarter_state_rm_ga
dev.off()

png(filename = "./plots/z-drugs-q1-2017-q1-2020-by-year-quarter-drug-no-ga.png", width = 16, height = 9, units = "in", res = 1200)
rx_by_year_quarter_drug_rm_ga
dev.off()